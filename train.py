...
import argparse
import numpy as np
from models.stgate import STGATE
...
parser.add_argument('--data', type=str, default='/content/drive/My Drive/Colab Notebooks/STGATE/data/METR-LA/')
parser.add_argument('--adj_filename', type=str, default='/content/drive/My Drive/Colab Notebooks/STGATE/data/METR-LA/adj_mx_dijsk.pkl')
parser.add_argument('--params_dir', type=str, default='/content/drive/My Drive/Colab Notebooks/STGATE/experiment_METR_LA')
...
def main():
    ...
    net = STGATE(args.cuda, args.num_of_vertices, args.num_of_features, args.points_per_hour*args.num_of_hours, args.num_for_predict)
    if args.cuda:
        net = net.cuda()
    ...
    n_steps_per_epoch = int(len(dataloader['train_loader']) / args.batch_size)
    ...
    for epoch in tqdm(range(args.epoch)):
        ...
        # + train_rmse = []
        t1 = time.time()
        for step, (trainx, trainy, trainx_) in enumerate(dataloader['train_loader']):
            optimizer.zero_grad()
        ...
        with torch.no_grad():
            ...
            val_metrics = {
              "val/val_loss": valid_loss / len(dataloader['val_loader']), 
              "val/val_mape": valid_mape / len(dataloader['val_loader']),
              "val/val_rmse": valid_rmse / len(dataloader['val_loader']), 
              }
            
            s2 = time.time()
            val_metrics['val/time'] = s2-s1
            ...
            if args.early_stop_maxtry < trycnt:
                print('early stop!')
                torch.save(net, args.params_dir+"_epoch_"+str(epoch)+"_"+str(round(mvalid_loss, 2))+".pth")
                wandb.finish()
                return
